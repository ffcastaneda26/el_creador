...
class PdfController extends Controller
{
    public function index($record, string $document = "aviso")
    {
        switch ($document) {
            case 'aviso':
                return $this->aviso_pricacidad($record);
            case 'cotizacion':
                return $this->cotizacion($record);
            case 'contrato':
                return $this->contrato($record);
            default:
                return response()->json(['error' => 'Documento no soportado: ' . $document], 400);
        }
    }

    /**
     * Crea aviso de privacidad para un cliente
     *
     * @param mixed $record: ID del cliente
    ...
    public function aviso_pricacidad($record)
    {
        ...
        if (!file_exists($filePath)) {
            \Log::error('Archivo PDF no encontrado: ' . $filePath);
     ...
     *
     * @param mixed $record: ID de la cotización
     * @return \Illuminate\Http\Response
     */
            ...
            if ($data && $i == 1) {
                ...
                if ($totImagesCotization) {
                    ...
                    foreach ($data->images as $image) {
                        // Usar la ruta absoluta en el servidor
                        ...
                        $fpdi->Image($imagePath, $posx, $posy, 15, 15);
     ...
     *
     * @param mixed $record: ID de la orden
     * @return \Illuminate\Http\Response
     */
