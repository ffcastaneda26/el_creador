...
<tr class="border-b dark:border-gray-700 hover:bg-gray-100">
    ...
    <td class="px-6 2 whitespace-nowrap text-end text-sm font-medium">
        <input wire:model="quantity"
            ...
            required min="1"
            max="{{ $ware_request_detail->quantity - $ware_request_detail->quantity_delivered }}"
            {{ $ware_request_detail->hasPending() ? '' : 'disabled' }}
        />

    </td>
    <td class="px-6 2 whitespace-nowrap text-center text-sm font-medium">
        @if($quantity <=  $ware_request_detail->quantity - $ware_request_detail->quantity_delivered &&  $quantity > 0)
            <x-button wire:click="updateItem()"
                    ...
                    :disabled="$quantity > $ware_request_detail->quantity - $ware_request_detail->quantity_delivered">
                    {{   __('Suply')  }}
            </x-button>
        @else
